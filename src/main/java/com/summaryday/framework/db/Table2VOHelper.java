...
public class Table2VOHelper {

	private static final Logger log = LoggerFactory.getLogger(Table2VOHelper.class);
	//基本数据配置
		private String packageOutPath = "com";// 指定实体生成所在包的路径 备注：执行前报名要先行创建好
		private String authorName = "林子";// 作者名字
		private String tablename = "tbl_test";// 表名
		 ...
		 * 
		 * @param _defaultPath
		 * @param _packageOutPath
		 * @param _tablename
		 * @return
		 */
		public boolean setTable2VO(String _defaultPath,String _packageOutPath,String _tablename) {
			boolean flag=false;
			FileWriter fw = null;
			PrintWriter pw =null;
		    if(!StringUtils.isBlank(_defaultPath)){this.defaultPath=_defaultPath;}
		    if(!StringUtils.isBlank(_packageOutPath)){this.packageOutPath=_packageOutPath;}
		    if(!StringUtils.isBlank(_tablename)){this.tablename=_tablename;}
		    else{
		    	log.error("你没有指定表名，请核实后再试！");
		    	return false;
		    }
			// 查要生成实体类的表
			...
			try {
				...
				try {
					...
					String outputPath = directory.getAbsolutePath() + this.defaultPath+ this.packageOutPath.replace(".", "/") + "/" + initcap(tablename) + ".java";
					...
					pw.flush();
					pw.close();
					log.info("执行完毕，生成路径为："+outputPath);
					flag=true;
				} catch (IOException e) {
					e.printStackTrace();
				}
