package com.summaryday.framework.xa;
...
/**
 * 关于XID的实现有个需要注意的细节，如果要保证数据的事务一致性，除了全局事务ID一致外，还必需保证formatid一致，
 * 这个formatid一般是不会变的，但不同的厂商一般是不一样的，在xid传播过程中，应该保证使用一样的formatid才可以。
...
public class XATest {
    public static XADataSource createXADataSource() throws Exception {
       ...
       // OracleXADataSource dataSource = new OracleXADataSource();
    	...
    	String url="jdbc:mysql://192.168.1.113:3306/cloudtraining?useUnicode=true&characterEncoding=utf8";
